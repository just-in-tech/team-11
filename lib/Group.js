 ...
 * @param {Pen} pen - The pen to use for drawing the group.
...
export class Group extends Array {
     ...
     * @property {Pen} pen - The pen to use for drawing the group.
    ...
    constructor(...elements) {
        ...
        if (this.id > 3) {
            this.fill =
                this.#approvedColours[
                    Math.floor(Math.random() * this.#approvedColours.length)
                ];
        } else {
            this.fill =
                this.#approvedColours[
    ...
    overlaps(collideableEntity) {
        ...
        if (collideableEntity === this) {
            return this.pen
                .getCollisionManager()
                .collidesOrOverlapsGroupSelfToSelf(this, this.id, IS_OVERLAPS);
        }
    ...
    collides(collideableEntity) {
        ...
        if (collideableEntity === this) {
            return this.pen
                .getCollisionManager()
                .collidesOrOverlapsGroupSelfToSelf(this, this.id);
        }
        if (collideableEntity instanceof Group) {
            return this.pen
                .getCollisionManager()
