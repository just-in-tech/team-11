...
export class BuildTreeScene {
    ...
    draw() {
        ...
        if ($.frameCount <= this.firstframe+600) {
            console.log(this.firstframe)

            $.text.size = 45
            $.colour.fill = "white"
            $.text.font = this.fonttitle
            $.text.print($.w / 2, 60, "BUILD YOUR TREE", 800)
        }
    }
        ...
        let varbranchname=branchname+"Branch";
            ...
            if(branchlevel<3){
                if(this.notices[varbranchname].display==="statstable"){
                    ...
                    $.text.font = this.font
                    $.text.size = 15
                    $.text.print($.w/2,textposition,"upgrade fibre production per interval to " + data.statsUpgrades[varbranchname][varbranchlevel].fibrePerInterval + " from "+data.playerStats.fibre.fibrePerInterval)
                    textposition+=18
                    $.text.print($.w/2,textposition,"upgrade fibre production interval to " + data.statsUpgrades[varbranchname][varbranchlevel].fibreInterval/60 + " (in seconds) from " + data.playerStats.fibre.fibreInterval/60)
